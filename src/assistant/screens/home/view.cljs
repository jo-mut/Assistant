...
(defn voice-stop []
  ...
  (-> (.stop rn/voice)
      (.then (fn []
               (println "Voice recognition stop")))
      (.catch (fn [error]
...
(defn- f-view []
  (fn []
    (react/useEffect
     (fn []
       (when (.-loaded rn/voice)
         (.onSpeechResults rn/voice (fn [results]
                                      (js/console.log "speech results: " results)))
         (.onSpeechError rn/voice (fn [error]
                                    (println "speech error: " error)))
         (.onSpeechPartialResults rn/voice (fn [results]
                                             (println "speech partial results: " results)))
         (.onSpeechEnd rn/voice (fn []
                                  (println "speech end"))))
       (fn []
         (.destroy rn/voice)
         (.removeAllListeners rn/voice)
         (.stopListening rn/voice)))
     []))
